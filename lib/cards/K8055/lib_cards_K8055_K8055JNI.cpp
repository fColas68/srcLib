 ...
 */
#include  "lib_cards_K8055_K8055JNI.h"
#include <usb.h>
#include <iostream>
#include <stdio.h>
#include <string.h>
...
JNIEXPORT jint JNICALL Java_lib_cards_K8055_K8055JNI_openDevice
  (JNIEnv* env, jobject thisObject, jlong BoardAddress)
{
    bool lTrouvee = false;

    if (DEBUG) {std::cout<<"Initialisation de la bibliothèque des ports USB"<<std::endl;}
    init_usb();
    if (DEBUG) {std::cout<<"Parcours des ports USB"<<std::endl;}
...
JNIEXPORT jlong JNICALL Java_lib_cards_K8055_K8055JNI_readAnalogChannel
  (JNIEnv* env, jobject thisObject, jlong Channel)
{
...
JNIEXPORT void JNICALL Java_lib_cards_K8055_K8055JNI_test
  (JNIEnv* env, jobject thisObject)
  {
      std::cout<<"JNI fonctionnel !"<<std::endl;
  }
